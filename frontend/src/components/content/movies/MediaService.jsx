...
async function searchMedia(req, res) {
    ...
    try {
        ...
        const tvResponse = await axios.get(`https://api.themoviedb.org/3/search/tv?api_key=${apiKey}&query=${query}&page=${page}&year=${year}&language=${language}`);
        ...
        const tvData = tvResponse.data.results.map(result => new tvModel(
            result.id,
            result.name,
            result.poster_path,
            result.overview
